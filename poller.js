...
try {
const solRes = BigInt(pumpData.virtual_sol_reserves);
const tokRes = BigInt(pumpData.virtual_token_reserves);
if (tokRes === 0n) return null;
return (Number(solRes) / 1e9) / (Number(tokRes) / 1e6) * solPrice;
} catch {
return null;
}
}
...
// Fetch SOL price once per cycle — reused for all pump.fun tokens
const solPriceUsd = await fetchSolPrice();

console.log(`[poll] Checking ${addresses.length} tokens — SOL: $${solPriceUsd || '?'}`);

for (const address of addresses) {
// Skip if already being processed (prevents double-fire on overlap)
...
// Save once at end for lastPrice / lastChecked updates
saveDB(db);
}
...
// Read flags with safe defaults for tokens tracked before this update
...
const bondingProgress = entry.bondingProgress ?? 0;
...
if (platform === ‘pumpfun’ && !entry.graduationAlertFired) {
...
if (pumpData.complete === true && !graduationAlertFired) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: 'Final bonding MCap', value: fmtUsd(pumpData.usd_market_cap), inline: true },
  ...
  return; // Next poll will use DexScreener
}
...
const newBondingProgress = pumpData.bonding_curve_progress || 0;
db.tokens[address].bondingProgress = newBondingProgress;

// ── Check F: 85% bonding curve alert ─────────────────────────────────
if (newBondingProgress >= 85 && !bondingAlertFired) {
  const embed = new EmbedBuilder()
    .setColor(0xff9900)
    .setTitle(`⚡ ${entry.name} (${entry.symbol}) — ${newBondingProgress.toFixed(0)}% to Raydium`)
    .setDescription(
      `**${entry.name}** is ${newBondingProgress.toFixed(0)}% through its bonding curve.\n\n` +
  ...
  console.log(`[bonding] ${entry.name} at ${newBondingProgress.toFixed(0)}%`);
}

// Reset bonding alert if dropped below 70
if (newBondingProgress < 70 && bondingAlertFired) {
...
// Update lastChecked even if no price
...
// ── RESET: if price dropped below 1.5x, reset all alerts ───────────────
...
saveDB(db); // Save reset immediately
console.log(`[reset] ${entry.name} dropped below 1.5x — all alerts reset`);
}
}

// Re-read flags after potential reset
const currentMilestonesFired = db.tokens[address].milestonesFired ?? [];
const currentTakeProfitFired = db.tokens[address].takeProfitFired ?? false;
const currentGainAlertFired = db.tokens[address].gainAlertFired ?? false;

// ── Check A: +75% gain alert ────────────────────────────────────────────
if (currentMultiple >= 1.75 && !currentGainAlertFired) {
const embed = new EmbedBuilder()
.setColor(0x00ff88)
.setTitle(`📈 ${entry.name} (${entry.symbol}) — up 75% from entry`)
.setDescription(
`**${entry.name}** is up 75% from the entry price.\n\n` +
...
// Save immediately — prevents double-fire
...
const latestFired = db.tokens[address].milestonesFired ?? [];
if (!latestFired.includes(milestone) && currentMultiple >= milestone) {
const embed = new EmbedBuilder()
.setColor(0xffd700)
...
```
  ...
  // Save IMMEDIATELY after each milestone — critical for preventing double-fire
  db.tokens[address].milestonesFired = [...(db.tokens[address].milestonesFired ?? []), milestone];
  saveDB(db);
...
// ── Check C: Take profit nudge at 2x ───────────────────────────────────
const latestTakeProfitFired = db.tokens[address].takeProfitFired ?? false;
if (currentMultiple >= 2.0 && !latestTakeProfitFired) {
const embed = new EmbedBuilder()
.setColor(0xff9900)
.setTitle(`💰 ${entry.name} (${entry.symbol}) is up ${currentMultiple.toFixed(2)}x`)
.setDescription(
`${entry.name} has doubled from entry.\n\n` +
`👀 Might be worth considering taking some off the table.\n\n` +
...
```
if (dexUrl) embed.setDescription(embed.data.description + `\n\n[DexScreener](${dexUrl})`);

await sendEmbed(client, entry.alertChannelId, embed);

// Save IMMEDIATELY — this is the fix for the double-post bug
...
// ── Check D: Dev wallet dump alert ─────────────────────────────────────
// (Placeholder — implement if Moralis dev wallet tracking is active)
// If devWallet is set, check holding % vs devHoldingAtCall
