...
// Returns a deduplicated array of unique addresses found in the message.
// This is the critical fix — we deduplicate BEFORE processing to prevent
// the same CA triggering two confirmations when it appears in both the
// message text AND an embedded pump.fun URL.
...
// Solana base58 addresses — must have a digit, no 0/O/I/l chars
// Also strip any trailing punctuation or URL fragments
...
// Skip if it looks like a common word or URL component
if (addr.length < 32) continue;
found.add(addr); // Solana addresses are case-sensitive, keep as-is
}
}

// Return as array — each address appears exactly ONCE
return […found];
}

// ─── Fetch token data (try DexScreener → pump.fun → skip) ─────────────────
...
// Try pump.fun (pre-graduation tokens)
try {
...
symbol: d.symbol,
chain: ‘solana’,
price: null, // calculated from virtual reserves
...
if (!token) return; // Not a real token — silently skip

// Calculate token age flag
const ageFlag = getTokenAgeFlag(token.pairCreatedAt);

// Calculate buy/sell pressure display
...
descParts.push(`⏳ Bonding curve: **${token.bondingProgress?.toFixed(0) || 0}%** to Raydium`);
}
...
console.log(`[detect] Found ${addresses.length} address(es) in message from ${message.author.username}: ${addresses.join(', ')}`);

// Process each unique address — one at a time to avoid race conditions
...
// Fetch live prices for all in parallel
...
return [
  ...
  `└ **${entry.postedBy}** · ${fmtTime(entry.postedAt)} · MCap: ${fmtUsd(live?.marketCap || entry.lastVolume)} · ${buyPct}% buys`,
].join('\n');
```
...
if (!db.tokens[address]) {
return interaction.reply(`❌ Not tracking \`${address}``);
}
...
// Start polling loop — every 3 minutes
pollTokens(client);
